...
export default function WriteThroughTestPage() {
  /* react-router */
  ...
  /* state */
  const [writeThroughLogs, setWriteThroughLogs] = useState<{
    chart: {
  ...
  const paginatedWriteThroughLogs = writeThroughLogs.logs.slice((pageTableLogs * 15) - 15, pageTableLogs * 15);
  const chunkedLabels = writeThroughLogs.chart.resource_utils.labels?.slice((chunkNumber * 10) - 10, chunkNumber * 10);
  const chunkedRespTime = writeThroughLogs.chart.resource_utils.datasets[0].data.slice((chunkNumber * 10) - 10, chunkNumber * 10);
  const chunkedResUtil = writeThroughLogs.chart.resource_utils.datasets[1].data.slice((chunkNumber * 10) - 10, chunkNumber * 10);

  /**
   * 1. Execute 50 requests, write 500 data per request using 50 different sampling
   * 2. Execute 50 requests, read 500 data per request using 50 different sampling before
   * 3. Execute 50 requests, update 500 data per request using 50 different sampling before
   * 4. Execute 50 requests, read updated 500 data per request using 50 different sampling before
   * 5. Execute 50 requests, combination write 500 data per request, following with read 500 new written data per request
   * @returns void
   */
  const RunWriteThroughScenario = async () => {
    setLoading(true);
    setDisplayLogs(true);
    ...
    const logHeaders = new Headers({
      "Authorization": "Bearer " + token,
      "X-Measure-Cache-Request-Logs": "write-through",
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data) {
        return setWriteThroughLogs(data);
      };
    })();
  }, [refetch]);
  return (
          ...
          >
            <Grid item xs={6}>
              ...
              >
                <Bar
                  data={{
                    ...writeThroughLogs.chart.cache_status,
                    datasets: [
                      {
                        ...writeThroughLogs.chart.cache_status.datasets[0],
                        minBarLength: 10,
                      }
                    ]
                  }}
            ...
            <Grid item xs={6}>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <Tooltip title="Next Logs" placement="bottom-end">
                      <IconButton size="small"
                        disabled={chunkNumber === (Math.ceil(writeThroughLogs.chart.resource_utils.datasets[0].data.length / 10))}
                        onClick={() => {
                          setChunkNumber(prev => prev + 1);
                        }}
                      >
                ...
                <Line
                  data={{
                    ...writeThroughLogs.chart.resource_utils,
                    labels: chunkedLabels,
                    datasets: [
                      {
                        ...writeThroughLogs.chart.resource_utils.datasets[0],
                        data: chunkedRespTime,
                        backgroundColor: green[200],
                        borderColor: green[400]
                      },
                      {
                        ...writeThroughLogs.chart.resource_utils.datasets[1],
                        data: chunkedResUtil,
          ...
          <Box component={"div"}>
            ...
            <TableContainer>
              ...
              >
                ...
                <TableBody>
                  {writeThroughLogs.logs.length < 5 && (
                    <TableRow>
                  ...
                  {paginatedWriteThroughLogs.map((log_, index) => (
              ...
              <Pagination
                size="small"
                count={Math.ceil(writeThroughLogs.logs.length / 15)}
                page={pageTableLogs}
